      ...
      if (responseObj.status == 200) {
        if (responseObj.headers['set-cookie'])
          requestHeader['Cookie'] +=
            responseObj.headers['set-cookie'].toString() + ';';
        ...
        reqURL = reqURL.indexOf('http') == -1 ? hostURL + reqURL : reqURL;

        method = 'get';
        requestBody = '';
        if (responseObj.headers['set-cookie'])
          requestHeader['Cookie'] +=
            responseObj.headers['set-cookie'].toString() + ';'; //TODO 수정 필요
        console.log('');
        continue;
      } else {
